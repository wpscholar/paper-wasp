...
/**
 * Class Paper_Wasp_Plugin_Check
 *
 * This class is responsible for performing basic checks to see if the minimum requirements
...
class Paper_Wasp_Plugin_Check {
	 ...
	 * @param string $file
	 */
	function Paper_Wasp_Plugin_Check( $file ) {
	...
	function check_has_min_php_version() {
		if ( version_compare( PHP_VERSION, $this->min_php_version, '<' ) ) {
			$error_msg = sprintf(
				__( '%s requires PHP version %s or later. You are currently running version %s.', 'paper-wasp' ),
				$this->name,
				$this->min_php_version,
				PHP_VERSION
			);
			$error_msg .= __( ' Please contact your web host about upgrading PHP.', 'paper-wasp' );
		...
		foreach ( $this->req_php_extensions as $extension ) {
			if ( ! extension_loaded( $extension ) ) {
				$this->errors->add(
					'php_extension',
					sprintf(
						__( '%s requires the %s PHP extension.', 'paper-wasp' ),
						$this->name,
						$extension
					)
				);
	...
	function check_has_min_wp_version() {
		...
		if ( version_compare( $wp_version, $this->min_wp_version, '<' ) ) {
			$this->errors->add(
				'wp_version',
				sprintf(
					__( '%s requires WordPress version %s or later. You are currently running version %s.', 'paper-wasp' ),
					$this->name,
					$this->min_wp_version,
					$wp_version
				)
	...
	function admin_notices() {
		...
		printf(
			__( '<p>The <strong>%s</strong> plugin has been deactivated.</p>', 'paper-wasp' ),
			$this->name
		);
		echo '</div>';
	}
